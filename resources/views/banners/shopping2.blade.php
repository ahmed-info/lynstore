
<div class="container">
    <div id="c4" class="carousel slide" data-bs-ride="carousel">
        <div class="carousel-indicators">
          <button type="button" data-bs-target="#c4" data-bs-slide-to="0" class="active" aria-current="true" aria-label="Slide 1"></button>
          <button type="button" data-bs-target="#c4" data-bs-slide-to="1" aria-label="Slide 2"></button>
          <button type="button" data-bs-target="#c4" data-bs-slide-to="2" aria-label="Slide 3"></button>
          <button type="button" data-bs-target="#c4" data-bs-slide-to="3" aria-label="Slide 4"></button>
        </div>
        <div class="carousel-inner">
            {{-- foreach --}}
          <div class="carousel-item active" style="height: 500px">
            <img src="../../images/banner-b1.jpeg" class="d-block w-100" alt="...">
            <div class="carousel-caption d-none2 d-md-block">
              <h5>Buy unique because you are unique</h5>
              <p><button class="btn text-white fw-bold fs-2">@lang('words.Shop Now')<i class="fa-solid fa-arrow-right"></i></button> </p>
            </div>
          </div>
          <div class="carousel-item" style="height: 500px">
            <img src="../../images/banner-b2.jpeg" class="d-block w-100" alt="...">
            <div class="carousel-caption d-none d-md-block">
                <h5>Buy unique because you are unique</h5>
                <p><button class="btn text-dark fw-bold fs-2">@lang('words.Shop Now')<i class="fa-solid fa-arrow-right"></i></button> </p>
            </div>
          </div>
          <div class="carousel-item" style="height: 500px">
            <img src=".../../images/banner-b3.jpeg" class="d-block w-100" alt="...">
            <div class="carousel-caption d-none d-md-block">
                <h5>Buy unique because you are unique</h5>
                <p><button class="btn text-dark fw-bold fs-2">@lang('words.Shop Now')<i class="fa-solid fa-arrow-right"></i></button> </p>
            </div>
          </div>

          <div class="carousel-item" style="height: 500px">
            <img src=".../../images/banner-b4.jpeg" class="d-block w-100" alt="...">
            <div class="carousel-caption d-none d-md-block">
                <h5>Buy unique because you are unique</h5>
                <p><button class="btn text-dark fw-bold fs-2">@lang('words.Shop Now')<i class="fa-solid fa-arrow-right"></i></button> </p>
            </div>
          </div>
        </div>
        {{-- <button class="carousel-control-prev" type="button" data-bs-target="#carouselExampleCaptions" data-bs-slide="prev">
          <span class="carousel-control-prev-icon" aria-hidden="true"></span>
          <span class="visually-hidden">Previous</span>
        </button>
        <button class="carousel-control-next" type="button" data-bs-target="#carouselExampleCaptions" data-bs-slide="next">
          <span class="carousel-control-next-icon" aria-hidden="true"></span>
          <span class="visually-hidden">Next</span>
        </button> --}}
      </div>
</div>


